...
export const WEAPONS = {
  ...
  wand: { name: '마법봉', icon: 'weapon_wand', pair: 'book', evo: '연쇄 천벌의 폭풍', evoIcon: 'evo_wand',
    ...
    // Different skill, different destiny: pairing with 'killer' (crit-focused)
    // instead of 'book' (cooldown-focused) evolves the SAME base weapon into
    // a completely different playstyle — one devastating guaranteed-crit
    // bolt instead of a chain-lightning AOE storm.
    altEvos: [{ pair: 'killer', evo: '치명의 마도서', evoIcon: 'evo_wand_alt',
      evoDesc: '적 하나에게 강력한 관통 마력탄을 발사합니다.',
      evoS: { dmg: 70, cd: 0.5, n: 1, guaranteedCrit: true } }] },
  ...
  boomerang: { name: '칼날 부메랑', icon: 'weapon_boomerang', pair: 'magazine', evo: '무한 회전날', evoIcon: 'evo_boomerang',
    ...
    // Reuses the exact same evolved firing code (FIRE.boomerang's w.evo
    // branch already just reads n/cd/size/dmg generically) — no weapons.js
    // change needed, just a faster/wider/lighter version of the same spin.
    altEvos: [{ pair: 'coil', evo: '폭풍의 검무', evoIcon: 'evo_boomerang_alt',
      evoDesc: '더 많은 칼날이 훨씬 빠르게 사방으로 회전합니다.',
      evoS: { dmg: 24, cd: 0.55, n: 8, size: 1.2 } }] },
...
// Normalizes a weapon instance's evolution data regardless of whether it
// evolved via the primary path or one of altEvos — everything downstream
// (HUD icon, build display, banners) reads through this instead of poking
// WEAPONS[w.id].evo*/altEvos directly, so adding more alt paths later never
// needs a second round of call-site changes.
export function evoInfo(w) {
  const D = WEAPONS[w.id];
  if (w.altIdx >= 0 && D.altEvos && D.altEvos[w.altIdx]) {
    const a = D.altEvos[w.altIdx];
    return { name: a.evo, icon: a.evoIcon, desc: a.evoDesc, stats: a.evoS };
  }
  return { name: D.evo, icon: D.evoIcon, desc: D.evoDesc, stats: D.evoS };
}
...
// Chest rarity tiers — index 0 unused (tiers are 1-based, see combat.js's
// chestTier()). Reused for both the reward count (openChest gives `tier`
// skills) and the color the chest renders/bursts as (render.js/growth.js).
export const CHEST_TIERS = [
  null,
  { name: '일반', color: '#cfcfcf' },
  { name: '고급', color: '#6ff3e8' },
  { name: '희귀', color: '#9b7bff' },
  { name: '전설', color: '#ffd166' },
];
